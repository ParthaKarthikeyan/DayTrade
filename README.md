...
|---|---|---|---|
| **Premarket stocks** | Momentum on low-float gappers, live Alpaca paper. Entry style (`pm_strategy`, since 2026-07-04): **first pullback** — squeeze → 1–5 red-candle pullback holding ≥50% of the leg → first candle to make a new high; stop = pullback low, target = HOD retest at ≥2:1, MACD gate ("flag" breakout entry still selectable). Daily governor: bank the day at **+1.5%**, hard stop at **-2%**, max 5 trades. Marketable-limit orders only. Bars (`PM_DATA_SOURCE`, since 2026-07-13): **yahoo** — polls the consolidated all-venue 1-minute tape (~1–2 min lag); the free IEX websocket left the bot blind on these names (36 bars/session on 2026-07-10). | weekdays 07:00–11:35 ET | `paper_ledger/premarket_*.json` |
